...
public class XmlParsing {
    ...
    public static Map<String, Object> parseXml(DispatchContext dctx, Map<String, Object> context) {
        ...
        Map<String, Object> solrResponse = sendItemsToSolr(itemsList);
    ...
    public static Map<String, Object> sendItemsToSolr(List<Map<String, Object>> itemsList) {
        String SOLR_CORE_URL = "http://localhost:8983/solr/pies";
        List<SolrInputDocument> solrDocs = new ArrayList<>();

        try (SolrClient solrClient = new HttpSolrClient.Builder(SOLR_CORE_URL).build()) {
            for (Map<String, Object> item : itemsList) {
                SolrInputDocument doc = new SolrInputDocument();

                doc.addField("productId", item.getOrDefault("PartNumber", ""));
                doc.addField("idValue", item.getOrDefault("ItemLevelGTIN", ""));
//                doc.addField("brandAAIAID", item.getOrDefault("BrandAAIAID", ""));
//                doc.addField("brandLabel", item.getOrDefault("BrandLabel", ""));
//                doc.addField("subBrandAAIAID", item.getOrDefault("SubBrandAAIAID", ""));
//                doc.addField("subBrandLabel", item.getOrDefault("SubBrandLabel", ""));
//                doc.addField("partTerminologyID", item.getOrDefault("PartTerminologyID", ""));

                // Description example: Pick description with DescriptionCode=PDD
//                List<Map<String, String>> descriptions = (List<Map<String, String>>) item.get("descriptions");
//                if (descriptions != null) {
//                    for (Map<String, String> desc : descriptions) {
//                        if ("PDD".equals(desc.get("DescriptionCode"))) {
//                            doc.addField("productDescription", desc.get("Description"));
//                            break;
//                        }
//                    }
//                }

                // You can also map prices, packages, digitalAssets etc if needed
                // Example for prices:
//                List<Map<String, String>> prices = (List<Map<String, String>>) item.get("prices");
//                if (prices != null && !prices.isEmpty()) {
//                    doc.addField("price", prices.get(0).get("Price")); // just the first price for simplicity
//                }
